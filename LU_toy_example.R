...
L1prop <- memoise(function(w, o, m) {
  JointProbaTable[w, o] * Sum(1:2, function(l) {
    ProbaLEX[l] * S1(m, o, l)
  })
})
...
Stable <- function(k) {
  table <- outer(1:4, 1:3, Vectorize(function(o, m) {
    S(k, m, o)
  }))
...
L <- memoise(function(k, w, o, m) {
  if (k == 1) {
    ...
    return(Lprop(k, w, o, m) / Lnorm(k, m))
  }
})
...
Ltable <- function(k) {
  table <- outer(1:2, 1:3, Vectorize(function(w, m) {
    Lw(k, w, m)
  }))
  colnames(table) <- c("m1", "m2", "m3")
