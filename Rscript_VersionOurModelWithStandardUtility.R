...
EpisUniform <- memoise(function(k) {
  function(n) {
    if (abs(4 - n) > k) {
      return(0)
    } else {
...
EpisPeaked <- memoise(function(k) {
  function(n) {
    if (abs(4 - n) > k) {
      return(0)
    } else {
      return(binom((2 * k), (n + k - 4), 0.5))
    }
  }
})
...
state <- memoise(function(x) {
  if (x < 5) {
    return(EpisUniform(x))
  } else {
    return(EpisPeaked(x - 4))
  }
})
...
Laround_prop <- memoise(function(n, e) {
  Num <- JointProba(n, e) * Sum(0:4, function(y) {
    test_between(n, y) * Prob_i(y)
  })
})
...
Lbetween_prop <- memoise(function(k) {
  function(n, e) {
    JointProba(n, e) * between(k)(n)
  }
})
Lbetween_norm <- memoise(function(k) {
  Sum(0:8, function(n) {
    Sum(0:8, function(e) {
      Lbetween_prop(k)(n, e)
    })
...
Lbetween <- memoise(function(k) {
  function(n, e) {
    Lbetween_prop(k)(n, e) / Lbetween_norm(k)
  }
})
...
L0 <- function(n, e, u) {
  if (u == 5) {
    ...
    return(Lbetween(u)(n, e))
  }
}

L0n <- memoise(function(n, u) {
  if (u == 5) {
    return(Sum(0:8, function(e) {
      Laround(n, e)
    }))
  } else {
    return(Sum(0:8, function(e) {
...
L <- memoise(function(k, n, e, u) {
  if (k == 0) {
    ...
    return(L_prop(k, n, e, u) / L_norm(k, u))
  }
})
...
U <- memoise(function(k, u, e) {
  Sum(0:(nworlds - 1), function(n) {
    if (P_n_given_e(n, e) == 0) {
      return(0)
    } else {
      return(P_n_given_e(n, e) * log(L((k - 1), n, e, u)))
    }
  }) - cost(u)
})
...
Ln_table <- function(k) {
  table <- outer(0:(nworlds - 1), 0:(nmessages - 1), Vectorize(function(n, u) {
    Ln(k, n, u)
  }))
...
Le_table <- function(k) {
  table <- outer(0:(nepis - 1), 0:(nmessages - 1), Vectorize(function(e, u) {
    Le(k, e, u)
  }))
  colnames(table) <- c("Exactly4", "b_3_5", "b_2_6", "b_1_7", "b_0_8", "around")
