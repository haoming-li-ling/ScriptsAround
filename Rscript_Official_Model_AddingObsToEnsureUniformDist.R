...
LaroundProp <- memoise(function(x) {
  Prob_n(x) * Sum(0:4, function(y) {
    test_between(x, y) * Prob_i(y)
  })
})
...
Lbetween <- memoise(function(k) {
  function(n) {
    Lbetween_prop(k)(n) / Lbetween_norm(k)
  }
})
...
L <- memoise(function(k, n, u) {
  if (k == 0) {
    ...
    return(L_prop(k, n, u) / L_norm(k, u))
  }
})
